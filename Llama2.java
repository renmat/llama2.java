...
public class Llama2 {
	...
	public Llama2(File weightsFolder) {
		...
		for (int i = 0; i < n_layers; i++) {
			...
			rms_ffn_weight[i] = (loadWeight(weightsFolder,
					"model.layers." + i + ".post_attention_layernorm.weight.npy"));
		...
		for (int lc = 0; lc < n_layers; lc++) {
			...
			for (int h = 0; h < n_heads; h++) {
				...
				for (int t = 0; t <= tokenPosition; t++) {
					...
					INDArray layer_token_k_cache_head = layer_token_k_cache
							.get(NDArrayIndex.interval(cacheOffset, cacheOffset + head_size));
					float score = (float) (Nd4j.matmul(q_head, layer_token_k_cache_head).getFloat()
							/ Math.sqrt(head_size));
					att.put(h, t, score);
				}
